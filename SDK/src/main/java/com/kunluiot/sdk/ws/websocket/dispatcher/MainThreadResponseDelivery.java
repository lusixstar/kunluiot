...
import static com.kunluiot.sdk.ws.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.BYTE_BUFFER_MSG;
import static com.kunluiot.sdk.ws.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.CONNECTED;
import static com.kunluiot.sdk.ws.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.CONNECT_FAILED;
import static com.kunluiot.sdk.ws.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.DISCONNECT;
import static com.kunluiot.sdk.ws.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.NON;
import static com.kunluiot.sdk.ws.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.PING;
import static com.kunluiot.sdk.ws.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.PONG;
import static com.kunluiot.sdk.ws.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.SEND_ERROR;
import static com.kunluiot.sdk.ws.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.STRING_MSG;
import static com.kunluiot.sdk.ws.websocket.util.ThreadUtil.checkMainThread;
import static com.kunluiot.sdk.ws.websocket.util.ThreadUtil.runOnMainThread;
...
public class MainThreadResponseDelivery implements ResponseDelivery {
    ...
    private static class CallbackRunnable<T> implements Runnable {
        ...
        public void run() {
            try {
                if (type == NON || mSocketListenerList == null || mSocketListenerList.isEmpty()) {
                    return;
                }
                //check null
                if (type == CONNECT_FAILED && connectErrorCause == null)
                    return;
                if (type == SEND_ERROR && errorResponse == null)
                    return;
                if (type == STRING_MSG && TextUtils.isEmpty(textResponse))
                    return;
                if (type == BYTE_BUFFER_MSG && byteResponse == null)
                    return;
                if (type == PING && framedataResponse == null)
                    return;
                if (type == PONG && framedataResponse == null)
                    return;
                synchronized (LISTENER_BLOCK) {
                    switch (type) {
                        case CONNECTED:
