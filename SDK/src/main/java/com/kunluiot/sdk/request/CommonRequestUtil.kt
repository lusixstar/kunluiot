...
object CommonRequestUtil {
    ...
    fun getCommonProblem(callback: ICommonProblemCallback) {
        Kalle.get(ReqApi.KHA_CONSOLE_BASE_URL + CommonApi.KHA_API_COMMON_PROBLEM).setHeaders(KunLuHelper.getSign()).perform(object : KunLuNetCallback<BaseRespBean<CommonProblemBean>>(KunLuHomeSdk.instance.getApp()) {
    ...
    fun feedback(username: String, title: String, content: String, images: String, contact: String, callback: IResultCallback) {
        ...
        Kalle.post(ReqApi.KHA_CONSOLE_BASE_URL + CommonApi.KHA_API_FEEDBACK).setHeaders(KunLuHelper.getSign()).addHeader("authorization", "Bearer " + KunLuHomeSdk.instance.getSessionBean()?.accessToken).body(JsonBody(param)).perform(object : KunLuNetCallback<BaseRespBean<CommonProblemBean>>(KunLuHomeSdk.instance.getApp()) {
                override fun onResponse(response: SimpleResponse<BaseRespBean<CommonProblemBean>, String>) {
                    val failed = response.failed()
                    if (!failed.isNullOrEmpty()) {
                        callback.onError(response.code().toString(), failed)
