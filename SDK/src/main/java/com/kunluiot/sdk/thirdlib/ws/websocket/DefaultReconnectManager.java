    ...
    private Runnable getReconnectRunnable() {
        return new Runnable() {
            ...
            public void run() {
                ...
                if (mWebSocketManager.getSetting().isAutoConnect()) {


                    while (true) {

                        LogUtil.d(TAG, "reconnectTimes = " + reconnectTimes);
                        if (reconnectTimes > 0 && reconnectTimes <= 10) {
                            try {
                                Thread.sleep(5 * 1000);
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        } else if (reconnectTimes > 10 && reconnectTimes <= 20) {
                            try {
                                Thread.sleep(10 * 1000);
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        } else if (reconnectTimes >= 20 && reconnectTimes < 35) {
                            try {
                                Thread.sleep(15 * 1000);
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        } else if (reconnectTimes >= 35) {
                            reconnectTimes = 0;
                        }
                    ...
                    try {
                        ...
                        for (int i = 0; i < count; i++) {
                            LogUtil.i(TAG, mWebSocketManager.getSetting().getConnectUrl() + "--" + String.format("第%s次重连", i + 1));
                            mWebSocketManager.reconnectOnce();
                            synchronized (BLOCK) {
                                try {
                                    BLOCK.wait(mWebSocketManager.getSetting().getConnectTimeout());
